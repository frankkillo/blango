...
class PostApiTestCase(TestCase):
    ...
    def test_post_list(self):
        ...
        for post_data in data:
            ...
            self.assertEqual(post_obj.slug, post_data["slug"])
            self.assertEqual(post_obj.summary, post_data["summary"])
            self.assertEqual(post_obj.content, post_data["content"])
            self.assertTrue(
                post_data["author"].endswith(
                    f"/api/v1/users/{post_obj.author.email}")
            )
            self.assertEqual(
                    post_obj.published_at,
                    datetime.strptime(post_data["published_at"], "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=UTC),
                )
